...
class SharedMLP(nn.Sequential):
    """PointNet-style shared 1x1 2D MLP used by coupled attention."""

    def __init__(self, channels, bn=False):
        super().__init__()
        for index in range(len(channels) - 1):
            in_channels, out_channels = channels[index:index + 2]
            self.add_module(
                'conv{}'.format(index),
                nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=not bn),
            )
            if bn:
                self.add_module('bn{}'.format(index), nn.BatchNorm2d(out_channels))
            self.add_module('relu{}'.format(index), nn.ReLU(inplace=True))
...
class AttentionEXTModule(nn.Module):
    def __init__(self, sum_or_maxpool: str='sum', if_softmax: bool=True, out_c: int=13):
        ...
        self.wei_conv = SharedMLP([256, 32, 64, 256], bn=True)
