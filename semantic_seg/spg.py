...
def spg_reader(args, fname, incl_dir_in_name=False, weakly_labeled=None):
    ...
    if f['sp_labels'].size > 0:
        ...
        num_sp = node_gt_size.shape[0]
        if weakly_labeled is not None:
            node_weak_label = np.argmax(node_gt_size[:, :args.n_labels], 1)[:, None]
            if not weakly_labeled:
                node_weak_label[:] = 255
        else:
            room_weak_labels = f['sp_weak_labels'][:].astype(np.int64)
            node_weak_label = np.argmax(room_weak_labels[:, :args.n_labels], 1)[:, None]
            node_weak_label[room_weak_labels[:,:args.n_labels].sum(1)==0,:] = 255

            for i in range(num_sp):
                if node_weak_label[i, 0] < 255:
                    sp_weak_labels = room_weak_labels[i, :]
                    mask = sp_weak_labels != 0

                    if np.sum(mask) <= 2:
                        weak_label = np.argmax(sp_weak_labels[:args.n_labels])
                        if sp_weak_labels[:args.n_labels].sum()==0:
                            weak_label = 255
                        node_weak_label[i, 0] = weak_label
                    else:
                        num_labels_in_sp = int(np.sum(mask)-1)
                        idx_labels = np.nonzero(sp_weak_labels)[0]
                        several_labels = idx_labels[:num_labels_in_sp]
                        corres_p_count = node_gt_size[i, several_labels]
                        node_weak_label[i, 0] = several_labels[np.argmax(corres_p_count, 0)]
    else:
