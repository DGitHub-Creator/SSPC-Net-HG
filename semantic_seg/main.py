...
def main():
    ...
    parser.add_argument('--ODPT_HG_PATH', default='/path/to/odpt-hg-sspc')
    parser.add_argument('--ODPT_HG_SPLIT_ROOT', default='/path/to/odpt-hg-data/splits')
    parser.add_argument('--odpt_budget', default=10, type=int, choices=[10, 20, 100])
    parser.add_argument('--odpt_split_file', default='', help='Explicit labeled Area 1 scene list')
    parser.add_argument('--odpt_steps_per_epoch', default=0, type=int,
                        help='Fixed ODPT-HG steps per epoch; 0 keeps minimum unlabeled coverage')
    parser.add_argument('--odpt_class_balance_power', default=0.0, type=float,
    ...
    parser.add_argument('--odpt_prototype_seeds', default=0, type=int,
                        help='Enable prototype-agreement seeds for fully unlabeled ODPT-HG scenes')
    parser.add_argument('--odpt_seed_confidence', default=0.85, type=float)
    parser.add_argument('--odpt_seed_margin', default=0.05, type=float)
    parser.add_argument('--odpt_seed_max_per_class', default=3, type=int)
    ...
    if args.dataset=='s3dis':
        ...
        create_train_dataset = lambda a: create_dataset(a)[0]
        create_test_dataset = lambda a, seed=0: create_dataset(a, seed)[1]
        args.n_labels = 13
    ...
    elif args.dataset=='odpt_hg':
        import odpt_hg_dataset
        dbinfo = odpt_hg_dataset.get_info(args)
        create_train_dataset = odpt_hg_dataset.get_train_dataset
        create_test_dataset = odpt_hg_dataset.get_test_dataset
        args.n_labels = 6
        if args.test_nth_epoch != 0:
            print('ODPT-HG protocol: disabling in-training Area 3 evaluation')
            args.test_nth_epoch = 0
    ...
    if args.dataset == 'odpt_hg':
        odpt_class_weights = torch.from_numpy(train_dataset.class_weights).float()
        if args.cuda:
            odpt_class_weights = odpt_class_weights.cuda()
        print('ODPT-HG labeled class point counts: {}'.format(
            train_dataset.class_point_counts.tolist()))
        print('ODPT-HG supervised class weights: {}'.format(
            train_dataset.class_weights.tolist()))
    else:
        odpt_class_weights = None
    test_dataset = None if args.dataset == 'odpt_hg' else create_test_dataset(args)
    ...
    def train(epoch):
        ...
        if args.dataset == 'odpt_hg':
            batch_sampler = odpt_hg_dataset.LabeledSceneBatchSampler(
                train_dataset.labeled_indices,
                train_dataset.unlabeled_indices,
                args.batch_size,
                args.seed + epoch,
                args.odpt_steps_per_epoch or None,
            )
        ...
        for bidx, (targets, GIs, clouds_data, clouds_orig, edges_for_ext, fnames, ext_data, num_sp_list) in enumerate(loader):
            ...
            weak_label_cat = weak_label_mode.clone()
            accumulated_ext_mask = ext_mask >= 0
            weak_label_cat[accumulated_ext_mask] = ext_weak_label_cuda[accumulated_ext_mask]

            if (epoch>0) and (epoch % args.ext_epoch_gap == 0):
                if args.dataset == 'odpt_hg' and args.odpt_prototype_seeds:
                    seed_idx, seed_label = prototype_seed_indices(
                        outputs, rnn_fea, weak_label_mode, num_sp_list,
                        confidence_th=args.odpt_seed_confidence,
                        prototype_margin=args.odpt_seed_margin,
                        max_per_class=args.odpt_seed_max_per_class,
                        ignore_label=args.ignore_label,
                    )
                    weak_label_cat[seed_idx] = seed_label
                else:
                ...
                weak_label2 = torch.as_tensor(
                    weak_label2, dtype=torch.long, device=outputs.device).reshape(-1)
                if not torch.is_tensor(output2) or output2.numel() == 0:
                    output2 = outputs.new_empty((0, outputs.shape[1]))
                else:
                    output2 = output2.to(outputs.device)
                if seed_idx.numel() > 0:
                    propagated_mask = ~(
                        extend_idx.reshape(-1, 1) == seed_idx.reshape(1, -1)
                    ).any(dim=1)
                    extend_idx = torch.cat((seed_idx, extend_idx[propagated_mask]), dim=0)
                    weak_label2 = torch.cat((seed_label, weak_label2[propagated_mask]), dim=0)
                    output2 = outputs[extend_idx, :]
                print('{}/{} ~ {:.2f} points with labels.'.format(outputs.shape[0], output1.shape[0], output1.shape[0]/outputs.shape[0]*100))
                print('prototype seed points: {}'.format(seed_idx.shape))
            ...
            loss1_cro = nn.functional.cross_entropy(
                outputs_valid1, weak_label1, weight=odpt_class_weights,
            ...
            labeled_idx = torch.nonzero(
                mask1, as_tuple=False).reshape(-1).long().to(outputs.device)
            # previous extension points:
            if torch.sum(accumulated_ext_mask) > 1:
                pre_ext_outputs = outputs[accumulated_ext_mask, :]    # Nsp_pre_ext
                pre_ext_fea = rnn_fea[accumulated_ext_mask, :]
                pre_ext_pseudo_label = torch.argmax(pre_ext_outputs, dim=1, keepdim=False)
                pre_ext_idx = torch.nonzero(
                    accumulated_ext_mask, as_tuple=False).reshape(-1).long().to(outputs.device)
            ...
            # extension concat
            if (torch.sum(accumulated_ext_mask) > 1) & (extend_idx.shape[0]>1):
            ...
            elif torch.sum(accumulated_ext_mask) > 1: # only previous extension points
            ...
            if (ext_idx_cat is not None) and (ext_idx_cat.shape[0]>20):
                ...
                ext_idx_idx = torch.arange(
                    ext_idx_cat.shape[0], dtype=torch.long, device=outputs.device)
    ...
    def eval_final():
        ...
        for ss in range(args.test_multisamp_n):
            test_dataset_ss = create_test_dataset(args, ss)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.test_nth_epoch > 0 and ((epoch+1) % args.test_nth_epoch == 0 or epoch+1==args.epochs):
    ...
    training_completed = args.epochs <= 0 or (
        len(stats) > 0 and stats[-1]['epoch'] == args.epochs - 1 and not math.isnan(stats[-1]['loss'])
    )

    # Final evaluation
    if args.test_multisamp_n>0 and (args.dataset != 'odpt_hg' or training_completed):
        if args.dataset == 'odpt_hg':
            print('ODPT-HG protocol: training complete; loading held-out Area 3 for final evaluation')
    ...
    elif args.dataset == 'odpt_hg' and args.test_multisamp_n > 0:
        print('ODPT-HG protocol: run did not complete; Area 3 final evaluation was skipped')
